...
function load_eg1(λ::T; n::Int=2, #λ::T=T(5.0), 
        σsq::T=T(0.1), 
        weight::Array{T}=T.([0.2, 0.8])) where T <: AbstractFloat
    ...
    U₁ = generate_mixGaussian(n,μlist,Σlist,weight)
...
function load_eg2(n::Int, λ::T; σsq₁::T=T(0.1), σsq₂::T=T(0.5)) where T<:AbstractFloat
    ...
    U₁ = generate_mixGaussian(n, center_pts, Σlist, weight)
...
function load_eg3(n::Int, σf::T; σ₀::T=T(1.0), radius=T(25.0)) where T<:AbstractFloat
    Ωq(x) = domain_ball(x, radius)
    U₀ = Gaussian(n, zeros(T,n), σ₀^2)
    UU₁ = Funnel(n, σf)
    exact_mean = σf/σ₀^n
    U₁ = RestrictedPotential(n, UU₁, Ωq) 
